...
def plan_available_in_my_area(plan:list, quad:int):
    ...
    if plan[1] == "wifi":
        if quad == 0 and footprint == "urban":
            return True
        if quad != 3 and footprint == "regional":
            return True
        if quad == 3 and footprint == "remote":
            return True
    else:
        if footprint == "remote":
            return True
...
def decide_bundle(person, complete_bundles, mobile_plans, wifi_plans, PrSacrificeWifi, IncomeBudget, Power, Seed, initial_call=False):
    '''function which chooses a person's bundle from a prepared set of options'''
    
    global add_to_seed
    #preference is to choose a complete bundle
    chosen_bundle = decision_tree(person, complete_bundles, IncomeBudget, Power, initial_call)
    if chosen_bundle != None:
        ...
        if random.random() < PrSacrificeWifi: 
            ...
            chosen_mobile_plan = decision_tree(person, mobile_plans, IncomeBudget, Power, initial_call)
        ...
        chosen_wifi_plan = decision_tree(person, wifi_plans, IncomeBudget, Power, initial_call)
...
def decision_tree(person, bundles, IncomeBudget, Power, initial_call):
    ...
    if initial_call or person[3] == (None, None):
        current_best = (1000000000, 0) #dummy bundle tuple
            ...
            if current_best_cost/income >= 0.05: #is current >= 5% of income?
                ...
                if new_cost/income < 0.05: #is new bundle < 5% of income?
                    if (new_cost/income <= 0.02) and (new_value > current_best_value): #is new bundle <= 2% of income and better value?
                        ...
                        if new_value/(new_cost**Power) > current_best_value/(current_best_cost**Power): #is new bundle better value for money?
                            current_best = bundle_evaluation_list[new_bundle]
                            current_best_index = new_bundle
...
def create_bundle_eval_list(bundles:list):
    ...
    if isinstance(bundles[0], tuple): 
        ...
        for bundle in bundles:
            ...
            bundle_evaluation_list.append((price, value))
...
def initialise_grid(TimeBudget:int, IncomeBudget:float, MarketingBudget:float, PrSacrificeWifi:float, Power:float, Seed:int):  
    ...
    for row in tqdm(range(100)):
        for cell in range(100):
            ...
            if is_populated:
                ...
                bundle = decide_bundle([location, True, income, (None, None), None], complete_bundles, mobile_plans, wifi_plans, PrSacrificeWifi, IncomeBudget, Power, Seed, True)
...
def all_agents_updates(grid, ISPs, replacements, dynamic_switching_costs, switchers, SwitchingCostIncrease, IncomeBudget, TimeBudget, MarketingBudget, PrSacrificeWifi, Power, Seed):
        ...
        for cell in range(100):
            if grid[row, cell, 1] == True:
                ...
                if bundle_update_trigger:
                    ...
                    new_bundle = decide_bundle(grid[row, cell], complete_bundles, mobile_plans, wifi_plans, PrSacrificeWifi, IncomeBudget, Power, Seed)
    ...
    for RA in range(3):
        for speed in range(6):
            if wifi_price_ra_this_timestep[RA][speed] != 0:
                wifi_price_ra_this_timestep[RA][speed] /= totals_wifi_price_ra[RA][speed]
    mobile_price_ra_this_timestep[RA] /= totals_mobile_price_ra[RA]
    for q in range(5):
        mobile_price_quint_this_timestep[q] /= totals_mobile_price_quint[q]
...
def simulate_market_dynamics(LargeMarkup, SmallMarkup, ResellerOperatingFee, WholesalerOperatingFee, MarketingBudget, PrPriceExp, LenPriceExp, PercentPriceChange, PrPlanExp, LenPlanExp, NumDissatisfied, SwitchingCostIncrease, TimeBudget, IncomeBudget, PrSacrificeWifi, Power, Seed, MaxIter):
    ...
    for i in tqdm(range(MaxIter)):
        ...
        num_real_problems, dynamic_switching_costs, switchers = all_agents_updates(grid, ISPs, replacements, dynamic_switching_costs, switchers, SwitchingCostIncrease, IncomeBudget, TimeBudget, MarketingBudget, PrSacrificeWifi, Power, Seed)
        if i%25 == 0:
...
def run_simulation(StartingMarket, InitialMoneyPool, LargeMarkup, SmallMarkup, ResellerOperatingFee, WholesalerOperatingFee, MarketingBudget, PrPriceExp, LenPriceExp, PercentPriceChange, PrPlanExp, LenPlanExp, NumDissatisfied, SwitchingCostIncrease, TimeBudget, IncomeBudget, PrSacrificeWifi, Power, Seed, MaxIter):
    ...
    grid = initialise_grid(TimeBudget, IncomeBudget, MarketingBudget, PrSacrificeWifi, Power, Seed)
    initial_grid = copy.deepcopy(grid)
    
    #simulation
    data = simulate_market_dynamics(LargeMarkup, SmallMarkup, ResellerOperatingFee, WholesalerOperatingFee, MarketingBudget, PrPriceExp, LenPriceExp, PercentPriceChange, PrPlanExp, LenPlanExp, NumDissatisfied, SwitchingCostIncrease, TimeBudget, IncomeBudget, PrSacrificeWifi, Power, Seed, MaxIter)
